...
const getPrevTrackIndex = () => {
  if (sharedAudioList.length === 0) return 0
  return (
    (sharedState.currentTrack - 1 + sharedAudioList.length) %
    sharedAudioList.length
  )
}
...
export const EndspacePlayer = ({ isExpanded, embedded = false }) => {
  ...
  if (!isExpanded) {
    return (
      <div
        className={`endspace-player-mini flex justify-center ${embedded ? 'py-0' : 'py-2'}`}>
        <div
          ...
          {isPlaying ? (
            ...
            <div
              className={`w-full h-full flex items-center justify-center text-[var(--endspace-text-muted)] hover:text-gray-600 hover:bg-gray-200 transition-all ${embedded ? 'rounded-full bg-transparent' : 'rounded-lg bg-[var(--endspace-bg-secondary)]'}`}>
      ...
      <div className='endspace-player-full relative flex h-full w-full items-center px-0.5'>
        ...
        <div className='flex min-w-0 flex-1 items-center gap-2'>
          <button
            ...
            />
            <span
              className={`absolute inset-0 flex items-center justify-center transition-opacity ${showPlaylist ? 'opacity-100' : 'opacity-0 group-hover/cover:opacity-100'}`}>
