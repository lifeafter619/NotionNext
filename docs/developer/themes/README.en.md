...
- **When merging into the upstream repo**: besides `themes/<id>/`, commit **`<id>.png`** and **`<id>.webp`** under **`public/images/themes-preview/`**, and add **`name` / `summary`** (and optional **`cover` / `coverWebp`**, **`tier`**) to **`THEME_SWITCH_MANIFEST`** in **`conf/themeSwitch.manifest.js`**; see [THEME_MIGRATION_GUIDE §8](../developer/THEME_MIGRATION_GUIDE.md) ([中文 §8](../developer/THEME_MIGRATION_GUIDE.zh-CN.md)). Local-only themes do not require this.
