...
产生的原因有两种1. URL配置错误，[https://waline.tangly1024.com](https://waline.tangly1024.com/) 结尾不能有斜杆。1. `Vercel`或`域名服务商`的问题导致触发了跨域限制。
...
![B60A41AC-6BC2-4F91-B3FB-FC742AD1BB27.png](/legacy/1a13c4dc8f6e6ea4.png) - 配置三个环境变量 `LEAN_ID`，`LEAN_KEY` ， `LEAN_MASTER_KEY` 。(它们的值分别对应上一步在 LeanCloud 中获得的 `APP ID`, `APP KEY`, `Master Key`。)，配置变量如下图：
