...
export function generateSitemap(allPosts = []) {
  const LINK = normalizeLink(siteConfig('LINK'))
  ...
  const postPages = allPosts
    ...
    .map(post => {
      const slugWithoutLeadingSlash = post.slug?.startsWith('/')
        ...
        : post.slug
      return {
        url: `${LINK}/${slugWithoutLeadingSlash}`,
        lastmod: safeISODate(post.lastEditedTime || post.date),
        changefreq: 'weekly',
        priority: '0.9'
      }
    })
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:wfw="http://wellformedweb.org/CommentAPI/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:sy="http://purl.org/rss/1.0/modules/syndication/" xmlns:slash="http://purl.org/rss/1.0/modules/slash/">
  <channel>
    ...
    <atom:link href="${LINK}/rss/feed.xml" rel="self" type="application/rss+xml"/>
    <link>${LINK}</link>
...
${publishedPosts
  .map(post => {
    const slugWithoutLeadingSlash = post.slug?.startsWith('/')
      ? post.slug.slice(1)
      : post.slug
    const postUrl = `${LINK}/${slugWithoutLeadingSlash}`
    const pubDate = safeUTCDate(post.publishDay || post.date)
    return `    <item>
...
export function generateSecurityTxt() {
  ...
  const LINK = normalizeLink(siteConfig('LINK'))
  const CONTACT_EMAIL =
    resolveContactEmail(siteConfig('CONTACT_EMAIL')) || 'security@example.com'
...
/**
 * 安全的ISO日期转换，避免Invalid Date
 * @param {*} dateInput
 * @returns {string}
 */
function safeISODate(dateInput) {
  try {
 ...
 * @returns {string}
 */
function safeUTCDate(dateInput) {
  try {
    const d = new Date(dateInput)
