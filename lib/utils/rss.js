...
const createFeedContent = async post => {
  ...
  if (blockMap) {
    const renderPost = { ...post, blockMap }
    // Notion修改了数据格式再次做统一兼容
    renderPost.blockMap = adapterNotionBlockMap(renderPost.blockMap)
    // 格式化内容，部分的样式字段格式在此处理
    if (renderPost.blockMap?.block) {
      renderPost.blockMap.block = formatNotionBlock(renderPost.blockMap.block)
    }

    const content = ReactDOMServer.renderToString(
      <ImageViewerProvider>
        <NotionPage post={renderPost} />
      </ImageViewerProvider>
    )
    const regexExp =
...
export async function generateRss(props) {
  ...
  let LINK = BLOG.LINK || siteInfo?.link
  // 确保链接不以斜杠结尾
  ...
  for (const post of latestPosts) {
    const slugWithoutLeadingSlash = post?.slug?.startsWith('/')
      ? post.slug.slice(1)
      : post.slug
    const postDate = new Date(post?.publishDate ?? post?.publishDay)
    feed.addItem({
      title: post.title,
