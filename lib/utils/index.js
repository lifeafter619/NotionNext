...
/**
 * Safely parse JSON from untrusted storage/config values.
 * @param {string|null|undefined} value
 * @param {*} fallback
 * @param {(parsed: any) => boolean} [isValid]
 * @returns {*}
 */
export function parseJsonWithFallback(value, fallback, isValid = () => true) {
...
/**
 * Decode URI components supplied by browsers or users without letting malformed
 * percent escapes break rendering.
 */
export function safeDecodeURIComponent(value, fallback = value) {
 ...
 */
const loadedResourcePromises = new Map()
...
export function loadExternalResource(url, type = 'js') {
  ...
  // 同一资源共享同一个加载 Promise：
  // 避免「脚本标签已插入但尚未加载完」时第二个调用方立即 resolve、
  // 拿到 undefined 的全局变量（如 window.QRCode / Prism 插件）
  const cacheKey = `${type}:${url}`
  if (loadedResourcePromises.has(cacheKey)) {
    // CSS 的 link 标签可能被外部移除（例如代码主题明暗切换），
    // 缓存命中时需确认标签仍在文档中，否则清除缓存重新加载
    const stillInDom =
      type !== 'css' || document.querySelector(`link[href='${url}']`)
    if (stillInDom) {
      return loadedResourcePromises.get(cacheKey)
    }
    loadedResourcePromises.delete(cacheKey)
  }

  // 检查是否已存在（非本函数插入的标签，视为已加载完成，维持旧行为）
  const elements =
    type === 'js'
  ...
  loadedResourcePromises.set(cacheKey, promise)
  // 加载失败时允许后续调用重试
  promise.catch(() => loadedResourcePromises.delete(cacheKey))
  return promise
}
