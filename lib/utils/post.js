...
import { hasAlgoliaAdminConfig } from '../plugins/algoliaConfig'
...
async function getPageAISummary(props, pageContentText) {
  ...
  try {
    // cache_manager 依赖 ioredis/fs 等服务端模块，本文件会进入客户端 bundle，
    // 因此只能在服务端执行路径中动态引入
    const [{ getDataFromCache, setDataToCache }, { getAiSummary }] =
      await Promise.all([
        import('@/lib/cache/cache_manager'),
        import('@/lib/plugins/aiSummary')
      ])
    ...
    // AI 摘要失败不应中断 processPostData（否则 prev/next/推荐文章都会丢失）
    console.warn('[AISummary] 获取文章摘要失败', error)
  }
}
...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0) {
    // 按 id 匹配而不是对象引用：通过 UUID 直达的文章是独立拉取的新对象，
    // indexOf 会返回 -1，slice(-2,-1) 会错误地取到倒数第二篇当“上一篇”。
    const index = allPosts.findIndex(
      p => p === props.post || (props.post?.id && p?.id === props.post.id)
    )
    if (index === -1) {
      props.prev = null
      props.next = null
    } else {
      props.prev = cleanPostListItemForClient(
        allPosts.slice(index - 1, index)[0] ?? allPosts.slice(-1)[0]
      )
      props.next = cleanPostListItemForClient(
        allPosts.slice(index + 1, index + 2)[0] ?? allPosts[0]
      )
    }
    props.recommendPosts = cleanPostListForClient(
      getRecommendPost(props.post, allPosts, siteConfig('POST_RECOMMEND_COUNT'))
    )
  } else {
    props.prev = null
    props.next = null
