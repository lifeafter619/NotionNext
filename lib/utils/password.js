...
export const getStoredPassword = key => {
  try {
...
export const setStoredPassword = (key, password) => {
  try {
    localStorage.setItem(key, password)
  } catch {}
}

export const rememberPasswordForPath = (path, password) => {
  setStoredPassword('password_' + getPasswordStoragePath(path), password)
}
...
export const getPasswordQuery = path => {
  ...
  if (queryParams.password) {
    ...
    setStoredPassword('password_default', queryParams.password)
  }
  ...
  const storedPassword = getStoredPassword('password_' + cleanedPath)
  const defaultPassword = getStoredPassword('password_default')
