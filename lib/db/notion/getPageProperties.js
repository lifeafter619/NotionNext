...
import { isMd5Digest, isSHA256Digest, sha256Digest } from '../../utils/password'
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      ...
      switch (schema[key]?.type) {
        case 'date': {
          // 声明为 date 但留空的列 getDateValue 返回 null，需判空避免 TypeError
...
function generateCustomizeSlug(postProperties, NOTION_CONFIG) {
  ...
  allSlugPatterns.forEach((pattern, idx) => {
    // 用数值 publishDate 而不是本地化字符串 publishDay：publishDay 会被
    // new Date() 按服务器本地时区解析，再读 UTC 字段会在非 UTC 服务器上
    // 差一天；publishDate 源自 start_date（UTC 午夜），UTC 字段稳定还原
    // 作者填写的日期，且不同时区的构建产物一致。
    if (pattern === '%year%' && postProperties?.publishDate) {
      const formatPostCreatedDate = new Date(postProperties?.publishDate)
      fullPrefix += formatPostCreatedDate.getUTCFullYear()
    } else if (pattern === '%month%' && postProperties?.publishDate) {
      const formatPostCreatedDate = new Date(postProperties?.publishDate)
      fullPrefix += String(formatPostCreatedDate.getUTCMonth() + 1).padStart(
        2,
        0
      )
    } else if (pattern === '%day%' && postProperties?.publishDate) {
      const formatPostCreatedDate = new Date(postProperties?.publishDate)
  ...
  const slug = postProperties.slug ?? postProperties.id
  if (fullPrefix) {
    return allSlugPatterns.includes('%slug%')
      ? fullPrefix
      : `${fullPrefix}/${slug}`
  } else {
    return `${slug}`
