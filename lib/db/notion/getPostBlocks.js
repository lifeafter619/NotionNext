...
export async function fetchNotionPageBlocks(
  ...
  // 放在缓存读取之后：旧缓存里缺 automation 的按钮页也能被补齐
  await addMissingButtonAutomations(pageBlock, cacheKey)
...
export function hasExpiredSignedUrls(recordMap, bufferMs = 10 * 60 * 1000) {
  ...
  return signedUrls.some(url => {
    try {
      const expirationTimestamp = new URL(url).searchParams.get(
        'expirationTimestamp'
      )
      if (!expirationTimestamp) return false

      const expires = Number(expirationTimestamp)
...
function getNotionFileSource(source) {
  ...
  if (source.includes('notion.so/signed/')) {
    try {
      const decodedSource = decodeURIComponent(
        new URL(source).pathname.replace(/^\/signed\//, '')
      )
      return isNotionHostedAssetSource(decodedSource) ? decodedSource : null
    } catch {
      return null
    }
  }

  return isNotionHostedAssetSource(source) ? source : null
}
...
export function preferStablePdfSignedUrls(recordMap) {
  Object.values(recordMap?.block || {}).forEach(entry => {
    ...
    if (!isNotionHostedAssetSource(source)) {
      return
    }
...
/**
 * 补抓按钮块引用的 automation / automation_action 记录。
 * Notion 长页面分块加载时，loadPageChunk 只随首个 chunk 返回部分 automation，
 * 后续 chunk 通过 syncRecordValues 补 block 表但不带 automation，
 * 导致按钮渲染成 "Untitled button" 且没有可执行动作。
 */
export async function addMissingButtonAutomations(recordMap, cacheKey) {
  if (!recordMap?.block) return

  const missingAutomations = new Map()
  Object.values(recordMap.block).forEach(entry => {
    const block = getBlockValue(entry)
    if (block?.type !== 'button') return
    const automationId = block.format?.automation_id
    if (automationId && !recordMap.automation?.[automationId]) {
      missingAutomations.set(automationId, block.space_id)
    }
  })

  if (missingAutomations.size === 0) return

  try {
    let patched = false
    const automationRecords = await syncRecordValuesByTable(
      'automation',
      missingAutomations
    )
    if (Object.keys(automationRecords).length > 0) {
      recordMap.automation = {
        ...recordMap.automation,
        ...automationRecords
      }
      patched = true
    ...
    Object.values(recordMap.automation || {}).forEach(record => {
      const automation = getBlockValue(record)
      if (!Array.isArray(automation?.action_ids)) return
      automation.action_ids.forEach(actionId => {
        if (actionId && !recordMap.automation_action?.[actionId]) {
          missingActions.set(actionId, automation.space_id)
        }
      })
    })

    if (missingActions.size > 0) {
      const actionRecords = await syncRecordValuesByTable(
        'automation_action',
        missingActions
      )
      if (Object.keys(actionRecords).length > 0) {
        recordMap.automation_action = {
          ...recordMap.automation_action,
          ...actionRecords
        }
        patched = true
    ...
    // 回写缓存，命中旧缓存的按钮页无需每次请求都补抓
    if (patched && cacheKey) {
      await setDataToCache(cacheKey, recordMap, null)
    }
  } catch (err) {
    console.warn('[Notion automations] syncRecordValues failed:', err)
  }
}

async function syncRecordValuesByTable(table, idToSpaceId) {
  const requests = [...idToSpaceId.entries()].map(([id, spaceId]) => ({
    pointer: { table, id, spaceId },
    version: -1
  }))

  const response = await notionAPI.__call('fetch', {
    // 注意：/api/v3/syncRecordValues 已被 Notion 关闭（403），必须用 Main 变体
    endpoint: 'syncRecordValuesMain',
    body: { requests }
  })

  return response?.recordMap?.[table] || {}
}
...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; ) {
    ...
    if (b?.value?.value?.id) {
      ...
      // Drop permission placeholders and malformed blocks without a real block id.
      delete clonedBlock[blockId]
      blocksToProcess.splice(i, 1)
      continue
    }
    ...
    if (b?.value) {
      ...
      normalizeSyncedBlock(b.value)
      // normalizeCalloutBlock must run before normalizeOfficialBlockPayload:
      // the latter deletes blockValue[originalType] (e.g. blockValue.callout)
      // after extracting rich_text/color, so callout-icon handling would
      // otherwise see no callout object.
      normalizeCalloutBlock(b.value)
      normalizeOfficialBlockPayload(b.value)
    ...
    normalizeExternalObjectInstanceBlock(b?.value)
    ...
    if (
      ...
      isNotionHostedAssetSource(b?.value?.properties?.source?.[0]?.[0])
    ) {
      const oldUrl = b?.value?.properties?.source?.[0]?.[0]
      const newUrl = oldUrl.includes('notion.so/signed/')
        ? oldUrl
        : `https://notion.so/signed/${encodeURIComponent(oldUrl)}?table=block&id=${b?.value?.id}`
...
function normalizeExternalObjectInstanceBlock(blockValue) {
  if (blockValue?.type !== 'external_object_instance') return

  const format = blockValue.format
  if (!format || format.domain) return

  const sourceUrl = format.original_url || format.uri
  if (typeof sourceUrl !== 'string') return

  try {
    format.domain = new URL(sourceUrl).hostname
  } catch {
    // Ignore malformed Notion metadata; the renderer will skip unsupported data.
  }
}

function normalizeOfficialBlockPayload(blockValue) {
  if (!blockValue?.type || typeof blockValue !== 'object') return

  const originalType = blockValue.type
  const data = blockValue[originalType]
  if (!data || typeof data !== 'object') return

  const richText = data.rich_text || data.text
  if (Array.isArray(richText) && !blockValue.properties?.title) {
    blockValue.properties = {
      ...blockValue.properties,
      title: officialRichTextToNotionDecorations(richText)
    }
  }

  if (Array.isArray(data.caption) && !blockValue.properties?.caption) {
    blockValue.properties = {
      ...blockValue.properties,
      caption: officialRichTextToNotionDecorations(data.caption)
    }
  }

  if (typeof data.color === 'string' && data.color !== 'default') {
    blockValue.format = {
      ...blockValue.format,
  ...
  if (originalType.startsWith('heading_') && data.is_toggleable === true) {
    blockValue.format = {
      ...blockValue.format,
      toggleable: true
    }
  ...
  if (originalType === 'to_do' && typeof data.checked === 'boolean') {
    blockValue.properties = {
      ...blockValue.properties,
  ...
  if (originalType === 'code' && data.language) {
    blockValue.properties = {
      ...blockValue.properties,
  ...
  const sourceUrl = data.external?.url || data.file?.url
  if (sourceUrl && !blockValue.properties?.source) {
    blockValue.properties = {
      ...blockValue.properties,
      source: [[sourceUrl]]
    }
  ...
  delete blockValue[originalType]
}

function officialRichTextToNotionDecorations(richText) {
  return richText
    .map(item => {
      const text =
        item?.plain_text ||
      ...
      if (annotations.bold) decorators.push(['b'])
      if (annotations.italic) decorators.push(['i'])
      if (annotations.strikethrough) decorators.push(['s'])
      if (annotations.code) decorators.push(['c'])

      const href = item?.href || item?.text?.link?.url
      if (href) decorators.push(['a', href])

      return decorators.length > 0 ? [text, decorators] : [text]
    })
    .filter(([text]) => text)
}

function normalizeSyncedBlock(blockValue) {
  if (blockValue?.type !== 'synced_block') return

  const syncedFrom = blockValue.synced_block?.synced_from
  const sourceBlockId = syncedFrom?.block_id || syncedFrom?.id

  delete blockValue.synced_block

  if (sourceBlockId) {
    blockValue.type = 'transclusion_reference'
    blockValue.format = {
      ...blockValue.format,
  ...
  blockValue.type = 'transclusion_container'
}
