...
import {
  ...
  mapNotionAssetUrlToProxy,
  unwrapExternalNotionImageSource
} from '@/lib/notionAssetUrl'
import { siteConfig } from '../../config'

const HUMAN_BADGE_FILENAME = 'Written-By-Human-Not-By-AI-Badge-black.svg'
const LOCAL_HUMAN_BADGE =
...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (img.includes(HUMAN_BADGE_FILENAME)) return LOCAL_HUMAN_BADGE

  // Old caches may contain a custom/Notion image wrapper around a bookmark or
  // another third-party URL. Restore the real external URL and leave it alone.
  const externalSource = unwrapExternalNotionImageSource(img, BLOG.NOTION_HOST)
  ...
  // 代理 Worker 已支持 /icons/*.svg 同路径直通，换到自有域名合并连接并吃
  // 边缘缓存。直接返回：svg 不参与压缩，也不追加 t= 缓存参数
  if (/^https:\/\/(?:www\.)?notion\.so\/icons\//i.test(ret)) {
    return ret.replace(/^https:\/\/(?:www\.)?notion\.so/i, BLOG.NOTION_HOST)
  }
  // 相对路径 /icons/… 在上面已拼上 NOTION_HOST，同样直接返回
  if (ret.startsWith(`${BLOG.NOTION_HOST}/icons/`)) {
    return ret
  }

  const hasConverted = isNotionAssetProxyUrl(ret, BLOG.NOTION_HOST)
  const needConvert = !hasConverted && isNotionHostedAssetSource(ret)

  // Only Notion-hosted assets are allowed onto the custom proxy.
  if (needConvert) {
    ret =
      BLOG.NOTION_HOST +
      '/image/' +
    ...
    ret = mapNotionAssetUrlToProxy(ret, BLOG.NOTION_HOST)
  } else if (/^https?:\/\//i.test(ret)) {
    // Preserve ordinary external URLs byte-for-byte. In particular, do not add
    // cache-busting or image transformation parameters to signed third-party
