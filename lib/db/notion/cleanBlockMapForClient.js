export function cleanBlockMapForClient(blockMap) {
  ...
  stripWebhookActionSecrets(cleanedBlockMap.automation_action)
...
/**
 * 判断 blockMap 中是否包含代码块。
 * 兼容压缩（compact）与未压缩两种数据形态：压缩形态直接查类型字典，开销极小
 */
export function blockMapHasCode(blockMap) {
  if (!blockMap?.block) return false

  const compactTypes = blockMap[COMPACT_BLOCK_TYPES_KEY]
  if (Array.isArray(blockMap[COMPACT_BLOCK_IDS_KEY])) {
    return Array.isArray(compactTypes) && compactTypes.includes('code')
  }

  return Object.values(blockMap.block).some(
    entry => (entry?.value || entry)?.type === 'code'
  )
}
...
function packCompactBlockValue(
  ...
  if (
    typeof packedValue.type === 'string' &&
    typeToCompactRef.has(value.type)
  ) {
...
function stripWebhookActionSecrets(recordMap) {
  Object.entries(recordMap || {}).forEach(([id, record]) => {
    const value = getRecordValue(record)
    if (!['send_webhook', 'http_request'].includes(value?.type)) return
    recordMap[id] = {
      value: {
        id: value.id || id,
        type: value.type
