...
function handleDataBeforeReturn(db) {
  ...
  db.allPages = stripHeavyData(cleanPages(db.allPages, db.tagOptions))
  db.latestPosts = stripHeavyData(cleanPages(db.latestPosts, db.tagOptions))
  // 必须在使用完毕后才能进行清理
  ...
  return pages.map(item => {
    return {
      ...
      lastEditedDate: item.lastEditedDate
        ? new Date(item.lastEditedDate).getTime()
        : 0,
      lastEditedDay: item.lastEditedDay || '',
