...
const overwriteAlgoliaSearch = async (posts) => {
  ...
  const objects = posts.map(post => {
    return {
      ...
      content: truncate(post.content && typeof post.content === 'string' ? post.content : getPageContentText(post, post.blockMap), 10000) // 限制为10000字符以符合Algolia免费额度限制
    }
  })

  try {
    await algoliaIndex.replaceAllObjects(objects).wait()
    console.log('Algolia索引重置成功，共索引', objects.length, '篇文章')
  } catch (error) {
...
const uploadDataToAlgolia = async post => {
  ...
  if (needUpdateIndex) {
    const record = {
      ...
      createdTimestamp: getValidTimestamp(post.createdTime),
      lastEditedTimestamp: getValidTimestamp(post.lastEditedDate),
      content: truncate(post.content && typeof post.content === 'string' ? post.content : getPageContentText(post, post.blockMap), 30000) // 提高索引字符限制以支持更多内容，需注意Algolia套餐限制
    }
