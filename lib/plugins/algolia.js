...
const uploadDataToAlgolia = async post => {
  ...
  if (needUpdateIndex) {
    const record = {
      ...
      // 将创建时间转换为时间戳，便于排序
      createdTimestamp: post.createdTime ? new Date(post.createdTime).getTime() : 0,
      lastEditedTimestamp: post.lastEditedDate ? new Date(post.lastEditedDate).getTime() : 0,
