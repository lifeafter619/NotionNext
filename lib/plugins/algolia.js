...
const generateAlgoliaSearch = ({ allPages, force = false }) => {
  if (!hasAlgoliaAdminConfig(BLOG)) return
...
const overwriteAlgoliaSearch = async posts => {
  ...
  for (const post of posts) {
    ...
    const fullContent =
      post.content && typeof post.content === 'string'
        ? post.content
        : getPageContentText(post, post.blockMap)
  ...
  try {
    await index.replaceAllObjects(objects).wait()
    console.log('Algolia索引重置成功，共索引', objects.length, '个分块')
  } catch (error) {
    if (isBuildTimeCredentialError(error)) {
      console.warn('Algolia凭据不可用，已跳过构建期索引更新:', error)
      return
    }
    if (isTemporaryIndexQuotaError(error)) {
      console.warn(
        'Algolia原子替换因临时索引配额失败，改用清空后批量写入索引:',
        error
      )
      try {
        ...
        if (isBuildTimeCredentialError(fallbackError)) {
          console.warn(
            'Algolia凭据不可用，已跳过构建期索引更新:',
            fallbackError
          )
          return
        }
...
function isTemporaryIndexQuotaError(error) {
  const message = error?.message || ''
  return /too many indices/i.test(message)
}

function isBuildTimeCredentialError(error) {
  if (process.env.npm_lifecycle_event !== 'build') return false
  const message = error?.message || ''
  return (
    error?.status === 401 ||
    error?.status === 403 ||
    /invalid application-id|invalid api key|api key/i.test(message)
  )
}
    ...
    const record = {
      ...
      content: truncate(
        post.content && typeof post.content === 'string'
          ...
          : getPageContentText(post, post.blockMap),
        5000
      ) // 提高索引字符限制以支持更多内容，需注意Algolia套餐限制
    }
