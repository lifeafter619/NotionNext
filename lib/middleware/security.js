  ...
  for (const [key, rules] of Object.entries(schema)) {
    ...
    if (rules.minLength !== undefined || rules.maxLength !== undefined) {
      if (!Validator.isValidLength(value, rules.minLength, rules.maxLength)) {
        errors.push(
          `${fieldPath} length must be between ${rules.minLength || 0} and ${rules.maxLength || 'unlimited'}`
        )
      }
    }
    ...
    if (rules.min !== undefined || rules.max !== undefined) {
      if (!Validator.isValidNumber(value, rules.min, rules.max)) {
        errors.push(
          `${fieldPath} must be between ${rules.min || '-∞'} and ${rules.max || '∞'}`
        )
      }
    }
...
export function requestLogMiddleware() {
  return (req, res, next) => {
    ...
    res.end = function (...args) {
      ...
      // 记录请求日志
      console.log(
        `[${new Date().toISOString()}] ${req.method} ${req.url} ${statusCode} ${duration}ms - ${ip} - ${userAgent}`
      )
