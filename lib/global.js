...
// Clerk 登录态桥：动态加载，避免 @clerk/react 静态进入所有页面的 _app bundle
...
export function GlobalContextProvider(props) {
  ...
  const [appearanceMode, updateAppearanceMode] = useState(() =>
    normalizeAppearanceMode(defaultDarkMode)
  )
  ...
  // 登录验证相关：登录态由 ClerkUserBridge（动态加载）回填，
  // 未启用 Clerk 时视为"已加载、未登录"
  const enableClerk = process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
  const [clerkAuth, setClerkAuth] = useState({
    isLoaded: !enableClerk,
    isSignedIn: false,
    user: false
  })
  const { isLoaded, isSignedIn, user } = clerkAuth
  ...
  const setAppearanceMode = useCallback(newMode => {
    const normalizedMode = normalizeAppearanceMode(newMode)
    saveDarkModeToLocalStorage(normalizedMode)
    updateAppearanceMode(normalizedMode)
    applyAppearanceMode(normalizedMode, updateDarkMode)
  }, [])

  // Legacy one-click controls still toggle between explicit light and dark.
  const toggleDarkMode = useCallback(() => {
    setAppearanceMode(
      isDarkMode ? APPEARANCE_MODE.LIGHT : APPEARANCE_MODE.DARK
    )
  }, [isDarkMode, setAppearanceMode])
  ...
  // 首次加载成功
  useEffect(() => {
    initDarkMode(updateDarkMode, defaultDarkMode, updateAppearanceMode)
  ...
  useEffect(() => {
    if (appearanceMode !== APPEARANCE_MODE.SYSTEM) return
    return subscribeToSystemAppearance(updateDarkMode)
  }, [appearanceMode])
  ...
  return (
    <GlobalContext.Provider value={contextValue}>
      {enableClerk ? <ClerkUserBridge onChange={setClerkAuth} /> : null}
      {children}
