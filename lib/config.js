...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  if (
    key === 'THEME_SWITCH' &&
    hasConfigValue(process.env.NEXT_PUBLIC_THEME_SWITCH)
  ) {
  ...
  switch (key) {
    ...
    case 'UUID_REDIRECT':
      ...
      return convertVal(
        firstProvidedConfigValue(extendConfig[key], BLOG[key], defaultVal)
      )
    default:
  }
  ...
  if (global) {
    ...
    val = firstProvidedConfigValue(
      global.runtimeConfigOverrides?.[key],
      global.NOTION_CONFIG?.[key],
      global.THEME_CONFIG?.[key]
    )
  }

  if (!hasConfigValue(val)) {
    // 这里针对部分key做一些兼容处理
    switch (key) {
  ...
  val = firstProvidedConfigValue(val, extendConfig?.[key], BLOG[key])
...
/**
 * Read a configuration value that is explicitly boolean.
 * Notion tables commonly contain yes/no or on/off instead of JSON booleans;
 * callers should not pass those values through JSON.parse.
 */
export const siteConfigBoolean = (
  key,
  defaultVal = false,
  extendConfig = {}
) => {
  const value = siteConfig(key, defaultVal, extendConfig)
