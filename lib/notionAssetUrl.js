...
/**
 * Restores the canonical Notion origin for an image served through a custom
 * proxy host (e.g. https://img.cdn.example/image/<encoded>), or unwraps a
 * third-party URL hidden inside a Notion image wrapper. Returns null when the
 * source is neither a custom-proxied Notion image nor a wrapped external link.
 *
 * Used by the client-side fallback chain to fall back to the Notion origin
 * directly when a custom CDN/proxy fails.
 */
export function getOriginalNotionImageSource(source, notionHost) {
  ...
  try {
    ...
    if (
      ...
      configuredUrl.origin !== DEFAULT_NOTION_ORIGIN
    ) {
