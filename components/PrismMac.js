...
const PrismMac = () => {
  ...
  useEffect(() => {
    ...
    // 行号需在主题样式生效后重新校准行高，保留主题加载 Promise
    let prismThemeLoaded = null
    ...
    const renderCodeEnhancements = () => {
      ...
      try {
        ...
        const stopMermaid = renderMermaid(mermaidCDN, isDarkMode)
        stopMermaidObserver =
          typeof stopMermaid === 'function' ? stopMermaid : () => {}
    ...
    const loadCodeStyleSheets = () => {
      ...
      prismThemeLoaded = loadPrismThemeCSS(
        isDarkMode,
        prismThemeSwitch,
        prismThemeDarkPath,
        prismThemeLightPath,
      ...
      if (codeMacBar || codeCollapse) {
        ...
        Promise.resolve(prismThemeLoaded)
          .catch(err => {
    ...
    const initCodeEnhancements = () => {
      ...
      // 性能优化：页面没有代码块或 Mermaid 图时不加载任何资源
      const article = getNotionArticle()
      const hasCodeBlocks = Boolean(article?.querySelector('pre.notion-code'))
      const hasMermaidBlocks = Boolean(
        article?.querySelector('.language-mermaid')
      )
      if (!hasCodeBlocks && !hasMermaidBlocks) return false
  ...
  const observer = new MutationObserver(mutationsList => {
    ...
    for (const m of mutationsList) {
      if (m.type === 'childList' && m.addedNodes.length > 0) {
        if (
          m.target.classList?.contains('language-mermaid') ||
          (m.target.querySelector &&
            m.target.querySelector('.language-mermaid'))
        ) {
          shouldProcess = true
          break
        }
...
const wrapMermaid = svg => {
  ...
  zoomIndicator.className =
    'mermaid-zoom-indicator absolute top-3 left-3 px-2 py-1 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded text-xs text-gray-600 dark:text-gray-400 font-mono shadow border border-gray-200 dark:border-gray-700'
  ...
  controls.className =
    'mermaid-controls absolute bottom-3 right-3 flex gap-1.5 z-10 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-lg p-1.5 shadow-lg border border-gray-200 dark:border-gray-700'
  controls.innerHTML = `
  ...
  container.onmousedown = e => {
    if (
      ...
      e.target.closest('.mermaid-controls') ||
      e.target.closest('a')
    )
      return
    isDragging = true
