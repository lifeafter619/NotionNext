...
async function runBackfill({ autoYes = false } = {}) {
  ...
  console.log(
    `[backfill] 英文库共 ${enPosts.length} 篇，中文库共 ${zhPosts.length} 篇`
  )
  ...
  for (const z of zhPosts) {
    ...
    const candidates = enPosts
      ...
      .map(e => ({
        ...e,
        score: Math.max(
          similarity(z.slug, e.slug),
          similarity(z.title, e.title)
        )
      }))
    ...
    candidates.forEach((c, i) => {
      console.log(
        `  ${i + 1}. [score=${c.score.toFixed(2)}] ${c.title}  (slug: ${c.slug || '—'})`
      )
    })
    ...
    if (autoYes) {
      ...
      ans = (
        await ask(`选择配对编号？[1-${candidates.length}/n/skip]: `)
      ).trim()
    }
    ...
    await notion.updatePageProperties(z.id, {
      paired_with: {
        rich_text: [{ type: 'text', text: { content: chosen.id } }]
      }
    })
