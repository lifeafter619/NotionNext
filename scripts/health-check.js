...
function runCommand(command, description, silent = false, timeout = 30000) {
  try {
...
function checkVSCodeConfig() {
  ...
  if (!fs.existsSync('.vscode')) {
    log('ℹ️  VSCode 配置目录不存在，跳过本地编辑器配置检查', 'yellow')
...
function checkTests() {
  ...
  const testFiles = [
    ...
    { path: 'jest.environment.js', name: 'Jest jsdom 环境适配' }
  ]
...
function runQualityChecks() {
  ...
  checks.forEach(({ command, name }) => {
    log(`\n🔧 运行 ${name}...`, 'cyan')
    const result = runCommand(command, name, true, 120000)
...
function testBuild() {
  ...
  log('🔧 运行构建命令...', 'cyan')
  const result = runCommand('npm run build', '项目构建', true, 600000)
